...
function questions_init() {
	...
	elgg_register_plugin_hook_handler('container_permissions_check', 'object', 'questions_container_permissions_check_handler');
	
	add_group_tool_option('questions', elgg_echo("questions:groups:enable"), true);
...
/**
 * Container permissions check handler
 * Allows users to post answers to questions in groups
 * 
...
function questions_container_permissions_check_handler($hook, $entity_type, $returnvalue, $params) {
	...
	if (isset($params['subtype'], $params['container']))
	{
		...
		if ($params['subtype'] == 'answer' && $container instanceof ElggQuestion)
		{
			...
			else if ($parentContainer instanceof ElggUser && has_access_to_entity($container, $params['user']))
			{
				return true;
			}
		}
