...
function assemblies_init() {
    ...
    // notifications
    elgg_register_notification_event('object', 'assembly', array('create'));
    elgg_register_plugin_hook_handler('prepare', 'notification:publish:object:assembly', 'assembly_notify_message');
...
function assembly_notify_message($hook, $type, $message, $params) {
    ...
    $notification->subject = elgg_echo('assembly:notify:subject', array($entity->title), $language);

    // Message body for the notification
    $notification->body = elgg_echo('assembly:notify:body', array(
        $owner->name,
        $entity->title,
        $entity->getExcerpt(),
        $entity->getURL()
    ...
    $notification->summary = elgg_echo('assembly:notify:summary', array($entity->title), $language);
}
