...
foreach ($result as $num => $object) {
    ...
    $csv[] = utf8_decode($object->guid);
    $csv[] = utf8_decode($object->name);
    $csv[] = utf8_decode($object->getMembers(array('count' => true)));
    $csv[] = date('d/m/Y', $object->time_created);
    ...
    if (!empty($activity)) {
        ...
        $csv[] = date('d/m/Y H:i', $activity->time_created);
    } else {
